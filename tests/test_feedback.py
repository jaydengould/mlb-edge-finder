...
def test_refresh_historical_calls_fetch_with_force():
    from mlb_win_probability import feedback
    mock_df = _make_historical_df(["2026-04-01"])
    with patch("mlb_win_probability.feedback.fetch_historical", return_value=mock_df) as mock_fetch:
...
def test_run_feedback_loop_no_retrain(tmp_path):
    ...
    with patch("mlb_win_probability.feedback.refresh_historical", return_value=hist_df), \
         patch("mlb_win_probability.feedback.config.MODELS_DIR", tmp_path), \
         patch("mlb_win_probability.feedback.config.RETRAIN_THRESHOLD", 15), \
         patch("mlb_win_probability.feedback.model.save_model") as mock_save:
...
def test_run_feedback_loop_retrain(tmp_path):
    ...
    with patch("mlb_win_probability.feedback.refresh_historical", return_value=hist_df), \
         patch("mlb_win_probability.feedback.config.MODELS_DIR", tmp_path), \
         patch("mlb_win_probability.feedback.config.RETRAIN_THRESHOLD", 15), \
         patch("mlb_win_probability.feedback.build_training_set", return_value=mock_training_df) as mock_build, \
         patch("mlb_win_probability.feedback.model.train", return_value=(mock_clf, MagicMock(), MagicMock(), MagicMock(), MagicMock())), \
         patch("mlb_win_probability.feedback.model.calibrate", return_value=mock_clf), \
         patch("mlb_win_probability.feedback.model.evaluate", return_value={}), \
         patch("mlb_win_probability.feedback.model.save_model") as mock_save:
        result = feedback.run_feedback_loop(2026)
...
def test_run_feedback_loop_no_existing_model(tmp_path):
    ...
    with patch("mlb_win_probability.feedback.refresh_historical", return_value=hist_df), \
         patch("mlb_win_probability.feedback.config.MODELS_DIR", tmp_path), \
         patch("mlb_win_probability.feedback.config.RETRAIN_THRESHOLD", 15), \
         patch("mlb_win_probability.feedback.build_training_set", return_value=pd.DataFrame({"home_win": [1]})), \
         patch("mlb_win_probability.feedback.model.train", return_value=(mock_clf, MagicMock(), MagicMock(), MagicMock(), MagicMock())), \
         patch("mlb_win_probability.feedback.model.calibrate", return_value=mock_clf), \
         patch("mlb_win_probability.feedback.model.evaluate", return_value={}), \
         patch("mlb_win_probability.feedback.model.save_model") as mock_save:
