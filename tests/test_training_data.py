...
def test_build_training_set_signature():
    from mlb_win_probability import training_data
...
def test_historical_name_to_abbr_covers_all_30_teams():
    from mlb_win_probability.training_data import HISTORICAL_NAME_TO_ABBR
    expected_abbrs = {
...
def test_legacy_abbr_normalize_maps_oak_to_ath():
    from mlb_win_probability.training_data import _LEGACY_ABBR_NORMALIZE
    assert _LEGACY_ABBR_NORMALIZE["OAK"] == "ATH"


def test_load_training_set_raises_when_missing(tmp_path):
    from mlb_win_probability import training_data
    with patch("mlb_win_probability.training_data.config.DATA_PROCESSED_DIR", tmp_path):
...
def test_build_training_set_joins_home_and_away_stats(tmp_path):
    from mlb_win_probability import training_data
    with patch("mlb_win_probability.training_data.load_cached_historical", return_value=_make_hist()), \
         patch("mlb_win_probability.training_data.fetch_stats", return_value=_make_stats()), \
         patch("mlb_win_probability.training_data.fetch_pitcher_stats",
               return_value=_make_pitcher_stats()), \
         patch("mlb_win_probability.training_data.config.DATA_PROCESSED_DIR", tmp_path):
...
def test_build_training_set_raises_runtime_error_when_historical_missing(tmp_path):
    from mlb_win_probability import training_data
    with patch("mlb_win_probability.training_data.load_cached_historical",
...
def test_build_training_set_drops_unmapped_teams(tmp_path):
    ...
    with patch("mlb_win_probability.training_data.load_cached_historical", return_value=hist), \
         patch("mlb_win_probability.training_data.fetch_stats", return_value=_make_stats()), \
         patch("mlb_win_probability.training_data.fetch_pitcher_stats",
               return_value=_make_pitcher_stats()), \
         patch("mlb_win_probability.training_data.config.DATA_PROCESSED_DIR", tmp_path):
...
def test_build_training_set_applies_legacy_abbr_normalization(tmp_path):
    ...
    with patch("mlb_win_probability.training_data.load_cached_historical", return_value=hist), \
         patch("mlb_win_probability.training_data.fetch_stats", return_value=stats), \
         patch("mlb_win_probability.training_data.fetch_pitcher_stats",
               return_value=_make_pitcher_stats()), \
         patch("mlb_win_probability.training_data.config.DATA_PROCESSED_DIR", tmp_path):
        df = training_data.build_training_set([2024])
...
def test_build_training_set_cache_first(tmp_path):
    ...
    with patch("mlb_win_probability.training_data.load_cached_historical") as mock_hist, \
         patch("mlb_win_probability.training_data.config.DATA_PROCESSED_DIR", tmp_path):
        df = training_data.build_training_set([2024])
...
def test_build_training_set_multi_season_concatenates(tmp_path):
    ...
    with patch("mlb_win_probability.training_data.load_cached_historical", side_effect=mock_hist), \
         patch("mlb_win_probability.training_data.fetch_stats", return_value=_make_stats()), \
         patch("mlb_win_probability.training_data.fetch_pitcher_stats",
               return_value=_make_pitcher_stats()), \
         patch("mlb_win_probability.training_data.config.DATA_PROCESSED_DIR", tmp_path):
...
def test_build_training_set_includes_pitcher_sp_cols(tmp_path):
    ...
    with patch("mlb_win_probability.training_data.load_cached_historical",
         ...
         patch("mlb_win_probability.training_data.fetch_stats", return_value=_make_stats()), \
         patch("mlb_win_probability.training_data.config.DATA_RAW_DIR", tmp_path), \
         patch("mlb_win_probability.training_data.config.DATA_PROCESSED_DIR", tmp_path):
        df = training_data.build_training_set([2024], force=True)
...
def test_select_snapshot_date_returns_latest_preceding():
    from mlb_win_probability.training_data import _select_snapshot_date
    from datetime import date
...
def test_build_season_pitcher_nan_for_pre_snapshot_game(tmp_path):
    ...
    with patch("mlb_win_probability.training_data.load_cached_historical", return_value=hist), \
         patch("mlb_win_probability.training_data.fetch_stats", return_value=_make_stats()), \
         patch("mlb_win_probability.training_data.fetch_pitcher_stats", return_value=_make_pitcher_stats()), \
         patch("mlb_win_probability.training_data.config.DATA_RAW_DIR", tmp_path), \
         patch("mlb_win_probability.training_data.config.DATA_PROCESSED_DIR", tmp_path):
        df = training_data.build_training_set([2024], force=True)
