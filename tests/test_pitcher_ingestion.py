...
def test_fetch_pitcher_stats_signature():
    from mlb_win_probability import pitcher_ingestion
...
def test_load_cached_pitcher_stats_raises_when_missing(tmp_path):
    from mlb_win_probability import pitcher_ingestion
    with patch("mlb_win_probability.pitcher_ingestion.config.DATA_RAW_DIR", tmp_path):
...
def test_fetch_pitcher_stats_returns_expected_columns(tmp_path):
    from mlb_win_probability import pitcher_ingestion
    with patch("mlb_win_probability.pitcher_ingestion.statsapi.get",
               return_value=_make_stats_response()), \
         patch("mlb_win_probability.pitcher_ingestion.config.DATA_RAW_DIR", tmp_path):
...
def test_fetch_pitcher_stats_skips_zero_ip(tmp_path):
    ...
    with patch("mlb_win_probability.pitcher_ingestion.statsapi.get", return_value=response), \
         patch("mlb_win_probability.pitcher_ingestion.config.DATA_RAW_DIR", tmp_path):
        df = pitcher_ingestion.fetch_pitcher_stats(date(2025, 9, 28), force=True)
    assert len(df) == 1
...
def test_fetch_pitcher_stats_cache_first(tmp_path):
    ...
    with patch("mlb_win_probability.pitcher_ingestion.statsapi.get") as mock_get, \
         patch("mlb_win_probability.pitcher_ingestion.config.DATA_RAW_DIR", tmp_path):
...
def test_fetch_pitcher_stats_excludes_low_ip_pitchers(tmp_path):
    ...
    from mlb_win_probability import pitcher_ingestion, config
    response = {
        "stats": [{
            "splits": [
                {
...
def test_fetch_probable_starters_returns_expected_columns():
    from mlb_win_probability import pitcher_ingestion
    with patch("mlb_win_probability.pitcher_ingestion.statsapi.schedule",
...
def test_fetch_probable_starters_returns_empty_on_no_games():
    from mlb_win_probability import pitcher_ingestion
    with patch("mlb_win_probability.pitcher_ingestion.statsapi.schedule", return_value=[]):
        df = pitcher_ingestion.fetch_probable_starters(date(2025, 4, 22))
    assert len(df) == 0
...
def test_fetch_pitcher_snapshot_falls_back_to_season_stats_when_no_splits(tmp_path):
    ...
    with patch("mlb_win_probability.pitcher_ingestion.statsapi.get", side_effect=responses), \
         patch("mlb_win_probability.pitcher_ingestion.config.DATA_RAW_DIR", tmp_path):
