...
def test_temporal_split_train_has_no_holdout_season():
    from mlb_win_probability.temporal_eval import _temporal_split
...
def _run_with_mocks(tmp_path: Path, training_df: pd.DataFrame, force: bool = False) -> dict:
    ...
    import mlb_win_probability.temporal_eval as te
    ...
    with patch.object(te, "_load_training_csv", return_value=training_df), \
         patch("mlb_win_probability.temporal_eval.XGBClassifier") as MockXGB, \
         patch("mlb_win_probability.temporal_eval.calibrate", return_value=mock_clf), \
         patch("mlb_win_probability.temporal_eval.evaluate", return_value={
             "accuracy": 0.57, "roc_auc": 0.60, "log_loss": 0.68, "brier_score": 0.24,
             "n_test_samples": 60,
         }), \
         patch("mlb_win_probability.temporal_eval.simulate_bets", return_value=empty_backtest), \
         patch("mlb_win_probability.temporal_eval.sweep_market_efficiency", return_value=fake_sweep), \
         patch("mlb_win_probability.temporal_eval.compute_summary", return_value={
             "n_bets": 0, "n_wins": 0, "win_rate": 0.0, "total_pnl": 0.0,
             "roi_pct": 0.0, "avg_ev": 0.0, "max_drawdown": 0.0, "sharpe_ratio": 0.0,
         }), \
         patch("mlb_win_probability.temporal_eval.config") as mock_config:
...
def test_break_even_alpha_returns_none_when_never_negative():
    from mlb_win_probability.temporal_eval import _break_even_alpha
...
def test_run_skips_if_exists(tmp_path):
    ...
    import mlb_win_probability.temporal_eval as te
    with patch("mlb_win_probability.temporal_eval.config") as mock_config:
        mock_config.MODELS_DIR = tmp_path
