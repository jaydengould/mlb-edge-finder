...
![MLB Edge Finder dashboard showing the flagged-games table, 30-day history chart, holdout evaluation stats, and market-efficiency sweep](docs/assets/dashboard-screenshot.png)
...
`edge_finder.market_implied_prob()` converts American odds to a raw,
...
```bash
python -m mlb_edge_finder                            # today, cache-first
python -m mlb_edge_finder --date 2026-05-12          # specific date
python -m mlb_edge_finder --force                    # bypass all caches
...
python -m mlb_edge_finder.generate_site              # regenerate docs/index.html
```
...
├── edge_finder.py           # probability vs posted line; flag divergences
