...
**Threshold sweep & market-edge filter complete.** Phases 1–3, 4a–4c, 5, 6, 7, and 8 done. `compute_kelly()`, `__main__.py` CLI, probability calibration, `prob_flag`, GitHub Actions daily workflow, historical backtest, and threshold sweep done.
...
1. **Current season feedback loop** — fetch yesterday's game results daily (via `statsapi`), append to the rolling historical data, and periodically retrain the model on the expanded dataset. This makes the model adaptive rather than static mid-season. The GitHub Actions workflow is the natural place to add this: after running the pipeline, fetch yesterday's result, append to `historical_2025.csv`, and retrain if N new games have accumulated since the last retrain.

2. **Real-odds backtest** — replace synthetic −110/−110 market odds with actual historical bookmaker lines from The Odds API historical endpoint (`GET /v4/sports/baseball_mlb/odds-history?date=...`). This costs API credits (one request per day of history) but produces a genuine P&L curve against real market prices, which is far more credible than the synthetic baseline. Cache the fetched odds locally like all other raw data.

3. **Out-of-time model evaluation** — retrain the model excluding 2025 entirely, then evaluate on 2025 as a true temporal holdout. This avoids the random-split leakage concern and better simulates how the model would perform going forward. Compare results to the current cross-sectional backtest.
...
5. **Dashboard / web UI** — a simple read-only page that renders `outputs/edges_YYYY-MM-DD.csv` as a formatted table with a running P&L chart. Could be a GitHub Pages static site generated by the daily workflow alongside the CSV commit.
