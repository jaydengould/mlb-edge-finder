...
- **Live paper-trading backtest complete:** `backtest.grade_live_edges(outputs_dir, unit)` — grades the edges actually published in `outputs/edges_*.csv` against real final scores from `load_cached_historical()`. Real bookmaker odds, real results, no model re-scoring and no synthetic market. Doubleheaders (duplicate `(game_date, home_name, away_name)`) and not-yet-final games are dropped. Returns the same per-bet schema as `run_backtest()` (`_BET_COLS`, factored out of `_run_bet_loop`) plus `high_confidence`, so `compute_summary()` works directly. `OUTPUTS_DIR` constant added to `backtest.py`. **Result as of 2026-08-02 (278 graded bets since 2026-05-19): 44.2% win rate, −8.7% ROI, Sharpe −0.083, max drawdown $3,111.** Average `model_prob` on flagged bets is 0.657 vs a 44.2% realized win rate — the EV filter systematically selects games where the model is overconfident, and the `high_confidence` badge is *anti*-predictive (35.5% win rate, −21.8% ROI). This is the real-market counterpart to the synthetic-odds result and contradicts it; the synthetic +18–22% ROI was an artifact of the naive 50/50 market baseline. 2 new tests (232 total passing).
