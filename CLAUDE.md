# CLAUDE.md — MLB Win Probability

## Project Overview

Portfolio project: a calibrated game-level win probability model for MLB, with a daily automated pipeline and a committed prediction history. An XGBoost classifier (isotonic-calibrated) predicts home-team win probability; the pipeline compares that probability against the posted moneyline and flags games where the divergence clears a configurable threshold. The original hypothesis — that those flagged divergences are positive-EV bets — was tested and **rejected**; see `Evaluation Record` below before writing any performance claim.

## Current Phase

**Evaluation complete; results are negative and the README says so.** Phases 1–3, 4a–4c, 5, 6, 7, and 8 done. `compute_kelly()`, `__main__.py` CLI, probability calibration, `high_confidence` badge (replaces `prob_flag`), GitHub Actions daily workflow, historical backtest, threshold rebalance (EV_THRESHOLD=0.20, MIN_PROB_EDGE removed), `fetch_historical` resilience, feedback loop, time-matched pitcher snapshots, dashboard, and temporal out-of-time evaluation done.
...
- **Temporal out-of-time evaluation complete:** `temporal_eval.py` — `_temporal_split(training_df, holdout_season)` splits by `season` column (train: `season < holdout`, test: `season == holdout`). `_load_training_csv()` globs `training_*.csv` and picks the file with the earliest start season then latest end season (parsed from the filename). `run(holdout_season=2025, force=False)` trains a fresh XGBClassifier on 2019–2024 (75/25 fit/val split for calibration), calibrates with isotonic regression, evaluates on 2025 holdout, runs `simulate_bets()` on holdout, writes `models/temporal_eval_{holdout_season}.json` with model metrics + backtest summary + per-bet P&L series. `backtest.simulate_bets(clf, X_test, y_test, meta_df, ...)` extracted from `run_backtest()` so the bet loop is callable on pre-split data. `generate_site.py` updated: `_load_metrics` / `_load_pnl` replaced by `_load_temporal_eval(models_dir)` (globs `temporal_eval_*.json`); `generate()` signature changed to `(outputs_dir, models_dir, out_path)`; `_render_stats_html` and JS P&L chart read from temporal eval JSON; stats card subtitle shows "Trained XXXX–XXXX · YYYY holdout". Dashboard now shows a single credible evaluation story: temporal holdout rather than random-split. Results on 2025 holdout: 12,606 train rows (2019, 2021–2024), 2,444 test rows, ROC-AUC 0.555, 205 bets, 62.0% win rate, +18.3% ROI, Sharpe 0.197. Run via `python -m mlb_win_probability.temporal_eval [--holdout-season 2025] [--force]`. `.github/workflows/daily.yml` `prob_flag` bug fixed (header-only CSV now uses `high_confidence`). 8 new tests (220 total passing).
- **ROI reframe + market-efficiency sweep complete:** `backtest._prob_to_american` promoted to module level; `backtest._run_bet_loop(clf, X_test, y_test, meta_df, home_odds, away_odds, ...)` extracted from `simulate_bets` to accept per-game odds; `simulate_bets` delegates to it (behavior unchanged). `backtest.sweep_market_efficiency(clf, X_test, y_test, meta_df, alpha_grid=None, vig=0.0476, ...)` sweeps market efficiency α∈[0,1] — each game's market-implied home prob set to `0.5*(1-α) + model_prob*α`, vig added, converted to per-game American odds, bet loop run — returning `alpha, roi_pct, n_bets, win_rate`. `temporal_eval._break_even_alpha(sweep_df)` linearly interpolates the α where ROI crosses 0 (None if never). `temporal_eval.run()` writes `market_efficiency_sweep` + `break_even_alpha` to the JSON and drops `pnl_series`. `generate_site.py`: `_render_pnl_html`→`_render_efficiency_html` (canvas `efficiency-chart` reading `market_efficiency_sweep`); stats card renamed "Holdout Evaluation", leads with ROC-AUC + break-even α, tags ROI/win-rate as "(naive market)". `model.calibrate()` docstring corrected (FrozenEstimator). README gains a "Limitations & What I'd Do Next" section. The synthetic ROI (+22.27%, `models/temporal_eval_2025.json`) is measured against `simulate_market_odds(home_market_prob=0.5, vig=0.0476)` — an uninformed counterparty pricing every game −110/−110. It is a sanity check only and carries no information about real-world profitability; that label must travel with the number every time it is quoted, or the number gets dropped. Break-even α≈0.4284 measures the simulation's sensitivity to counterparty informedness, not any real market. Regenerated artifact: 12,599 train rows (2019, 2021–2024), 2,444 test rows. 10 new tests (230 total passing).

- **Live paper-trading backtest complete:** `backtest.grade_live_edges(outputs_dir, unit)` — grades the edges actually published in `outputs/edges_*.csv` against real final scores from `load_cached_historical()`. Real bookmaker odds, real results, no model re-scoring and no synthetic market. Doubleheaders (duplicate `(game_date, home_name, away_name)`) and not-yet-final games are dropped. Returns the same per-bet schema as `run_backtest()` (`_BET_COLS`, factored out of `_run_bet_loop`) plus `high_confidence`, so `compute_summary()` works directly. `OUTPUTS_DIR` constant added to `backtest.py`. **`scripts/grade_live.py` persists the result to `models/live_grading.json` (all bets + `high_confidence` subset: `n_bets`, `date_range`, `win_rate`, `roi`, `sharpe`, `max_drawdown`, `mean_model_prob_on_flagged_bets`, `realized_win_rate`). Read the numbers from that file and cite it — never hard-code them here or in the README; they go stale every day the workflow publishes.** Regenerate with `python scripts/grade_live.py`.
  - The persistent finding is a **calibration failure on the selected subset**: mean `model_prob` on flagged bets sits far above the realized win rate (≈8.7 standard errors at n=420 as of the 2026-09-04 regeneration; the gap has been ~21pp and stable as the sample grew from 290 to 420), and the `high_confidence` badge selects a *worse* subset than the full flagged set. That result is statistically decisive.
  - The **ROI is not**: the standard error on mean per-bet return is ≈5pp at n=420, so the negative ROI stays ≈1.4 SE from zero. It has sat at ≈1.4 SE across both regenerations. Do not present it as a demonstrated loss.
  - Mechanism is **not** established. Two candidates both predict the pattern and the repo cannot separate them: **(a) adverse selection on the model's residuals** — flagging maximum model/market divergence preferentially selects the model's largest errors, so the flagged subset is not a random draw from the holdout Brier; **(b) train/serve skew on team stats** — training rows carry end-of-season stats from `fetch_stats(date(season, 9, 28))` while live inference in May–August is served partial-season stats the model never saw in training, which are noisier and more extreme and so inflate confidence (this also predicts the `high_confidence` slice being worse). Separating them requires retraining on as-of-date team stats and re-grading — same fix as `Future Work` item 1. Neither is evidence about market efficiency.
  - **Name the two defects distinctly.** The **random-split temporal leakage** (`models/metrics_*.json`, 80/20 random split puts games from one season on both sides) and the **team-stat look-ahead** (end-of-season team features inside the test season) are different problems. The temporal holdout removes the first and not the second. Never write "leakage" unqualified.
...
## Evaluation Record — read before making any performance claim

Three standing facts. Do not write around them.

1. **There is no market baseline.** The model has never been scored against bookmaker-implied probabilities. The repo holds no historical odds — `data/raw/odds_2025-07-01.csv` is header-only (0 rows) and the Odds API historical endpoint needs a paid plan. No de-vigging is implemented anywhere; `win_probability.market_implied_prob()` returns a raw vig-inclusive probability and only feeds the `high_confidence` display flag. This is a gap in the evaluation, **not** a finding about the market. Never claim the market is efficiently priced, and never claim the vig was the binding constraint.
2. **Look-ahead in team stats inflates every holdout metric.** `training_data._build_season()` joins team batting/pitching from `fetch_stats(date(season, 9, 28))` — end-of-season values — so holdout rows carry the finished season's team performance. The 2025 holdout (ROC-AUC 0.5626, log loss 0.6836, `models/temporal_eval_2025.json`) beats a no-input constant (log loss 0.6899, base rate 0.5252) by only **0.0063 log loss** — small enough that the team-stat look-ahead could plausibly account for all of it, and the repo cannot bound that look-ahead to rule it out. The holdout establishes an upper bound, not a measured skill level. Corroboration: the 2026 partial holdout (`models/temporal_eval_2026.json`) is ROC-AUC 0.5259 / log loss 0.7066, *worse* than its constant (0.6926). The same defect also produces a **train/serve skew**: live inference is served partial-season team stats the model never saw in training. Fixing this — time-matched team-stat snapshots, mirroring the pitcher-snapshot pattern — is the top priority in `Future Work` and addresses both symptoms.
3. **Cite files, never prose.** Every number in README.md and CLAUDE.md must come from `models/temporal_eval_2025.json`, `models/temporal_eval_2026.json`, `models/live_grading.json`, `models/metrics_*.json`, or `data/processed/training_2019-2026.csv`. Numbers copied into prose go stale; the audit that prompted this rewrite found exactly that.
...
Flat `src/mlb_win_probability/` package, installed in editable mode (`pip install -e .`). No sub-packages. One module per pipeline stage.

```
odds_ingestion → stats_ingestion → features → model → win_probability
                                                           ↑
                                                       pipeline.run()
```
...
| `win_probability.py` | Compute EV, filter odds, flag edges | `data/processed/edges_YYYY-MM-DD.csv` |
| `pipeline.py` | Orchestrate all stages end-to-end | — |
| `backtest.py` | Simulate historical performance on held-out test split with synthetic odds; grade real published edges against real results | — |
...
- **Error handling:** `odds_ingestion` and `stats_ingestion` raise `RuntimeError` on API failure. `model` raises `FileNotFoundError` if inputs are missing. `win_probability` logs a warning and returns an empty DataFrame when no edges are found — it never raises on empty results.
...
- **Kelly seam:** `win_probability.compute_ev(prob, american_odds)` is the EV primitive. When adding Kelly sizing, add `compute_kelly(ev, bankroll)` alongside it — don't modify `compute_ev`.
...
Implemented in `win_probability.compute_ev()`. This is the only fully implemented business logic function so far.
...
(or statsapi)                                                         ▼
                                                               train() / load_model()
                                                                      │
                                                                      ▼
                                                               select_flagged_games() ──► edges_YYYY-MM-DD.csv
```
...
232 smoke + integration tests. All pass.
...
- [x] Implement `win_probability.select_flagged_games()`
...
- [x] Add `compute_kelly()` to `win_probability` — half-Kelly sizing, `kelly_fraction` column in `select_flagged_games()` output.
- [x] Add `__main__.py` CLI entry point — `python -m mlb_win_probability [--date YYYY-MM-DD] [--force]`.
...
- [x] Add `high_confidence` column to `select_flagged_games()` output — `True` when `ev > HIGH_CONFIDENCE_EV (0.40)` and `model_prob − market_implied_prob > HIGH_CONFIDENCE_PROB_EDGE (0.15)`. Replaces old `prob_flag` warning; dashboard renders ★ badge.
...
Reordered after the evaluation came back negative. Items 1 and 2 both block any
honest performance claim; nothing below them is worth doing first.

1. **Time-matched team stats** — apply the pitcher-snapshot approach to team batting/pitching (`stats_ingestion` + `training_data._build_season`). Every training game for a season currently uses September 28 end-of-season team stats, so holdout rows carry the finished season's team performance. This is the **team-stat look-ahead** — leakage *inside the test season* — and it inflates every metric in `models/temporal_eval_*.json`. It is a different defect from the **random-split temporal leakage** in `models/metrics_*.json`; the temporal holdout removes the latter only. The fix mirrors `pitcher_ingestion.fetch_pitcher_snapshot`: capture team stats at the same four dates per season, commit them, and join each game to its latest preceding snapshot via `_select_snapshot_date`. It also closes the **train/serve skew**: training rows carry settled end-of-season team rates while live inference in May–August is served noisier, more extreme partial-season rates — candidate mechanism (b) for the live calibration failure. Retraining on as-of-date team stats and re-grading the published history is what would separate mechanism (a) from (b). Until this lands, no holdout number is a real out-of-sample figure and the cause of the live overconfidence stays unresolved.

2. **Real-odds baseline** — the model has never been scored against the market. Fetch historical bookmaker lines (The Odds API historical endpoint `GET /v4/sports/baseball_mlb/odds-history?date=...`; free tier returns 401, needs a paid plan), de-vig them by normalizing the two sides to sum to 1, and compare log loss / Brier against the model on the same games. This is the missing benchmark, not a P&L exercise — a de-vigged closing line is the standard baseline for this problem. Cache fetched odds locally like all other raw data.

3. **Fix selection, not just calibration** — the model is adequately calibrated in aggregate (Brier 0.2453 vs a 0.2484 constant) and badly calibrated on the games it flags (see `models/live_grading.json`). Flagging maximum model/market divergence selects on the model's own residuals — candidate mechanism (a). Investigate a selection rule that does not, or drop flagging and ship the probability model alone. Do this after item 1, which rules mechanism (b) in or out.

4. **Additional features** — rest days (days since last game), travel distance, ballpark factors, weather. Worth doing only after item 1: while the team-stat look-ahead is present, a new feature's apparent lift cannot be distinguished from that look-ahead.
